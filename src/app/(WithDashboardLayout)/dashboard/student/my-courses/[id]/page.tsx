...
interface SingleCourseContentDashboardPageProps {
  params: { id: string };
}
...
const SingleCourseContentDashboardPage: React.FC<SingleCourseContentDashboardPageProps> = ({ params }) => {
  const [course, setCourse] = useState<null | { title: string; lessons: ILessons[] }>(null);
  const [selectedLessonIndex, setSelectedLessonIndex] = useState<number>(0);

  useEffect(() => {
    async function fetchCourse() {
      const { data } = await getSingleCourse({ id: params.id });
      setCourse(data);
