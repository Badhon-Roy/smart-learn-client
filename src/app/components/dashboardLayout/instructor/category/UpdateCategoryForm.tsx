...
const UpdateCategoryForm = ({ category }: { category: ICategory }) => {
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (data) => {
        const toastLoading = toast.loading("Updating Category...")
        console.log(data);
        try {
            const res = await updateCategory(category?._id as string, data)
            if (res.success) {
    ...
    return (
        <div className="container mx-auto p-4 w-full">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl text-amber-500 font-bold">Update Category</h2>
